...
![The Exascale-class HPE Cray EX Supercomputer at Oak Ridge National Laboratory (fastest as of June 2024)](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Frontier_Supercomputer_%282%29.jpg/585px-Frontier_Supercomputer_%282%29.jpg)
:::
::: {.column width="33%"}
High Throughput Computing Cluster (HTC Cluster)

![Open Science Grid Consortium (OSG) <https://osg-htc.org>](fig/OSG_Map.png)
:::
::: {.column width="33%"}
![](https://upload.wikimedia.org/wikipedia/commons/thumb/4/4b/HTCondor_Logo.png/585px-HTCondor_Logo.png)
:::
:::
...
*   High Performance Computing Cluster (HPC Cluster): **Multiple** machines within
    a **single** network.
    
*   High Throughput Computing Cluster (HTC Cluster): **Multiple** machines across **multiple**
    networks.
...
![Source: Original figure from LUMI consortium documentation [@lumi2023]](https://raw.githubusercontent.com/gvegayon/appliedhpcr/e33949f25184bed5e84b02c356656782ddd5a46b/fig/socket-core-threads.svg)
...
- In the following example, we'll see the overhead cost of parallelization. Fitting $y = \alpha + \beta_k X_k + \varepsilon,\quad k = 1, \dots$
:::

::: {.columns}
::: {.column width="35%" style="font-size:80%"}

| Run | Description | Data sent | Data received |
|-----|-------------|-----------|---------------|
| 1   | Serial      | -  | - |
| 2   | Parallel full  | $X_k$ column  | Full model |
| 3   | Parallel coef  | $X_k$ column  | Coefficients |
| 4   | Parallel coef lite | indices only  | Coefficients |

In the last run, indexing occurs in the cluster side, where all the data is available.
:::
::: {.column width="50%"}
```{r}
#| label: overhead-cost
...
for (i in seq_along(nvars)) {
  ...
  timing_serial_lm_all[[i]] <- timer(x[, 1:nvars[i]], \(x., y) lm(y ~ x.))
  timing_parallel_lm_all[[i]] <- timer(x[, 1:nvars[i]], \(x., y) lm(y ~ x.), cl)

  timing_serial_lm[[i]] <- timer(x[, 1:nvars[i]], \(x., y) coef(lm(y ~ x.)))
  timing_parallel_lm[[i]] <- timer(x[, 1:nvars[i]], \(x., y) coef(lm(y ~ x.)), cl)

  timing_parallel_lm_lite[[i]] <-  system.time({
    parSapply(cl, 1:nvars[i], \(j) coef(lm(y ~ x[,j])))
  })
...
# Visualize
op <- par(lwd=2, cex=2)
maxy <- max(c(timing_serial_lm_all[, "elapsed"], timing_parallel_lm_all[, "elapsed"], timing_parallel_lm[, "elapsed"], timing_parallel_lm_lite[, "elapsed"]))
plot(timing_serial_lm_all[, "elapsed"] ~ nvars, type="b", col=1, pch=19, xlab="Number of variables", ylab="Time (s)", lty=1, ylim=c(0, maxy))
lines(timing_parallel_lm_all[, "elapsed"] ~ nvars, type="b", col=2, pch=19, lty=2)
lines(timing_parallel_lm[, "elapsed"] ~ nvars, type="b", col=3, pch=19, lty=3)
lines(timing_parallel_lm_lite[, "elapsed"] ~ nvars, type="b", col=4, pch=19, lty=4)

legend("topleft", legend=c("Serial", "Parallel: send & retrieve all", "Parallel send all & only get coefs", "Parallel: send once & only get coef"), col=1:4, lty=1:4, bty="n")
```
