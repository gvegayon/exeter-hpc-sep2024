...
# Visualize
op <- par(lwd=2, cex=1.5, mai = par("mai") * c(1.5,1.5,0,0))
...
cost_pll_coef_idx <- system.time({
  parSapply(cl, 1:ncol(X), \(j) coef(lm(y ~ X[,j])))
})

# Stopping the cluster
stopCluster(cl)

# Bonus: Fork cluster (starts and stops on the fly!)
...
data.frame(
  Type = c(
    "Serial", "Parallel", "Parallel: send all",
    "Parallel: send once", "Parallel: Fork"
    ),
  Elapsed = c(
    cost_serial[3], cost_pll[3], cost_pll_coef[3],
    cost_pll_coef_idx[3], cost_pll_fork[3]
    )
) |> t() |> knitr::kable()
```
