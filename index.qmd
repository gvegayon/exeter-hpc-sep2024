...
:::
::: {.column width="60%"}
![](fig/pll-computing-explained-serial.svg){width=80%}
:::
:::
...
:::
::: {.column width="60%"}
![](fig/pll-computing-explained-parallel.svg){width=80%}
:::
:::
...
![Ask yourself these questions before jumping into HPC!](fig/when_to_parallel.svg){fig-align="center" fig-alt="Ask yourself these questions before jumping into HPC!" width="65%"}
...
Finally, since we are only returning the coefficients, we can use a lighter version of `lm` called `lm.fit`.
...
cost_pll_lite <- system.time(
  parLapply(cl, 1:ncol(X), function(i) coef(lm.fit(y = y, x=X[,c(1, i),drop=FALSE]))
  )
)
