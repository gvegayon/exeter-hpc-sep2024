...
![Vadikus, CC BY-SA 4.0 <https://creativecommons.org/licenses/by-sa/4.0>, via Wikimedia Commons](https://upload.wikimedia.org/wikipedia/commons/thumb/c/ce/SIMD2.svg/466px-SIMD2.svg.png)
:::
:::
...
for (i in seq_along(nvars)) {
  ...
  timing_parallel_lm_lite[[i]] <-  system.time({
    mclapply(1:nvars[i], \(j) coef(lm(y ~ x[,j])), mc.cores = 4)
  }) * 1000
...
plot(timing_serial_lm_all[, "elapsed"] ~ nvars, type="b", col=1, pch=19, xlab="Number of variables", ylab="Time (ms) (log-scale)", lty=1, ylim=maxy, log="y")
